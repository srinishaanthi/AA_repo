...
export default function InvoiceForm({ editId, onNav }: Props) {
  ...
  const handleDownload = async () => {
    ...
    document.title = `${form.invoice_number || 'invoice'}`;
    window.print();
  ...
  const downloadPDF = () => {
    ...
    const opt = {
      margin: 0,
      filename: `Invoice_${form.invoice_number || 'New'}.pdf`,
    ...
    <div className="h-full flex flex-col bg-white">
      ...
      <div className="no-print bg-white border-b border-gray-100 px-6 py-3 flex items-center justify-between flex-shrink-0">
        ...
        <div className="flex items-center gap-2">
          ...
          <button onClick={handleDownload} className="btn-secondary"><Printer size={15} /> Print</button>
